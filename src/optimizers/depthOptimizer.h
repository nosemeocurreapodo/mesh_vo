...
public:
    ...
    DepthOptimizer(bool printlog = false)
        : Base(printlog),
    ...
    Vecx<float> get_params(const std::vector<Frame> &frames, const KeyFrame &kframe) const
    {
        std::vector<float> depths = get_depths(kframe.mesh());

        Vecx<float> params(numParams_);

        for (size_t i = 0; i < numDepths_; i++)
        {
            params(i) = fromDepthToParam(depths[i]);
        }
