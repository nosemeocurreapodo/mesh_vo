...
public:
    ...
    keyFrameGPU(const dataCPU<imageType> &im, vec2f _globalExp, SE3f _globalPose, float _globalScale)
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, points.size()*3*sizeof(float), points.data(), GL_STREAM_DRAW);
    ...
    keyFrameGPU(const keyFrameGPU &other)
    {
    ...
    keyFrameGPU &operator=(const keyFrameGPU &other)
    {
        if (this != &other)
        {
            globalPose = other.globalPose;
