...
TEST(PoseEstimatorTest, ComputePose)
{
    ...
    visualOdometryThreaded odometry(w, h, false, false);
    ...
    for (unsigned int i = 0; i < image_files.size(); i++)
    {
        ...
        if (i == 0)
        {
            ...
            odometry.init(imageData, depthData, pose, cam);
        }
        else
        {
            auto startTime = std::chrono::high_resolution_clock::now();
            SE3f estimatedPose = odometry.localize(imageData, pose);
            ...
            float error = computeSE3Error(estimatedPose, pose);
